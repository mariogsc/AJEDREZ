...
void Tablero::DibujaTablero() {
	...
	for (int columna=0; columna < NCasillas; columna++) {
		...
		for (int fila=0; fila < NCasillas; fila++) {
			...
			if (columna % 2 == 0) {
				if (fila % 2 == 0) casilla[columna][fila].SetColor(255,255,255);
				else casilla[columna][fila].SetColor(255, 0, 0);
			}
			else {
				if (fila % 2 == 0)casilla[columna][fila].SetColor(255, 0, 0);
				else casilla[columna][fila].SetColor(255, 255, 255);
			}
